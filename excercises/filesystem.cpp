...
int main() {
    ...
    std::string path1 = "./test3/test.txt"; 
    auto path2 = "./test3/test.txt";
    // bool out = fs::file_size(path1) != fs::file_size(path2);
    // auto out = fs::file_size(fs::current_path() / "test.cpp");
    std::error_code ec{};
    auto out = fs::file_size(path1, ec);
    std::cout << " >>> " << out << " " << ec;
