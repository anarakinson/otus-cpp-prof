...
void path_loop(po::variables_map &vm, std::function<void(fs::directory_entry, fs::directory_entry)> comparator) {

    std::set<fs::directory_entry> checked_obj;
    for (const std::string &path1 : vm["scan"].as<std::vector<std::string>>()) {
        for (const auto &obj1 : fs::directory_iterator(path1)) {
            
            for (const std::string &path2 : vm["scan"].as<std::vector<std::string>>()) {
